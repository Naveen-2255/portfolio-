...
export default function Sidebar({ onReboot }) {
  ...
  return (
    <>
      ...
      {/* MOBILE TOP BAR (Brand + Direct Resume Download - NO HAMBURGER MENU) */}
      <header className="md:hidden sticky top-0 z-40 bg-[#f8f5f2] border-b-2 border-black px-4 py-2.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 font-mono text-xs">
          {onReboot && (
            <button
              onClick={onReboot}
              className="px-2 py-1 bg-black text-green-400 border border-black font-bold flex items-center gap-1 text-[11px]"
              title="Reboot System Animation"
            >
              <RotateCcw size={12} /> BOOT
            </button>
          )}
          <a
            href="/resume.pdf"
            download
            className="px-2 py-1 bg-white border border-black font-bold flex items-center gap-1 text-[11px] hard-shadow-sm"
          >
